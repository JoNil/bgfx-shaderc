...
namespace bgfx
{
	...
	Options::Options()
		: shaderType(' '), disasm(false), raw(false), preprocessOnly(false), depends(false), debugInformation(false), avoidFlowControl(false), noPreshader(false), partialPrecision(false), preferFlowControl(false), backwardsCompatibility(false), warningsAreErrors(false), keepIntermediate(false), optimize(false), optimizationLevel(3)
	{
	}
...
#undef HEX_DUMP_FORMAT
	...
	struct Preprocessor
	{
		Preprocessor(const char *_filePath, bool _essl, bx::WriterI *_messageWriter)
			: m_tagptr(m_tags), m_scratchPos(0), m_fgetsPos(0), m_messageWriter(_messageWriter)
		{
		...
		{
			char temp[1024];
			bx::snprintf(temp, BX_COUNTOF(temp), "#ifndef %s\n"
												 "#	define %s 0\n"
												 "#endif // %s\n"
												 "\n",
						 _name, _name, _name);
		...
		void addInclude(const char *_includeDir)
		{
			...
			for (bx::StringView split = bx::strFind(start, ';'); !split.isEmpty(); split = bx::strFind(start, ';'))
			{
	...
	void help(const char *_error = NULL)
	{
		...
		bx::printf(
			...
			"Copyright 2011-2023 Branimir Karadzic. All rights reserved.\n"
			"License: https://github.com/bkaradzic/bgfx/blob/master/LICENSE\n\n",
			BGFX_SHADERC_VERSION_MAJOR, BGFX_SHADERC_VERSION_MINOR, BGFX_API_VERSION);
	...
	bool compileShader(const char *_varying, const char *_comment, char *_shader, uint32_t _shaderLen, const Options &_options, bx::WriterI *_shaderWriter, bx::WriterI *_messageWriter)
	{
		...
		{
			bx::snprintf(glslDefine, BX_COUNTOF(glslDefine), "BGFX_SHADER_LANGUAGE_GLSL=%d", profile->id);
		}

		char hlslDefine[128];
		if (profile->lang == ShadingLang::HLSL)
		{
			bx::snprintf(hlslDefine, BX_COUNTOF(hlslDefine), "BGFX_SHADER_LANGUAGE_HLSL=%d", profile->id);
		}
		...
		else if (0 == bx::strCmpI(platform, "osx"))
		{
			...
			char temp[256];
			bx::snprintf(
				temp, sizeof(temp), "BGFX_SHADER_LANGUAGE_METAL=%d", (profile->lang == ShadingLang::Metal) ? profile->id : 0);
		...
		else if (0 == bx::strCmpI(platform, "windows"))
		{
			...
			else if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
			{
				preprocessor.setDefine(glslDefine);
			}
		...
		while (!parse.isEmpty())
		{
			...
			if (!eol.isEmpty())
			{
				...
				if (0 == bx::strCmp(typen, "lowp", 4) || 0 == bx::strCmp(typen, "mediump", 7) || 0 == bx::strCmp(typen, "highp", 5))
				{
					precision = typen;
					typen = nextWord(parse);
				}

				if (0 == bx::strCmp(typen, "flat", 4) || 0 == bx::strCmp(typen, "smooth", 6) || 0 == bx::strCmp(typen, "noperspective", 13) || 0 == bx::strCmp(typen, "centroid", 8))
				{
					if ('f' == _options.shaderType || profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
					{
						interpolation = typen;
				...
				if (!typen.isEmpty() && !name.isEmpty() && !semantics.isEmpty())
				{
					Varying var;
					...
					if (profile->lang == ShadingLang::HLSL && profile->id < 400 && var.m_semantics == "BITANGENT")
					{
		...
		else if ('c' == _options.shaderType) // Compute
		{
			...
			else
			{
				...
				if (preprocessor.run(input))
				{
					...
					{
						bx::write(
							_shaderWriter, preprocessor.m_preprocessed.c_str(), (int32_t)preprocessor.m_preprocessed.size(), &err);
		...
		else // Vertex/Fragment
		{
			...
			else
			{
				if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
				{
					...
					for (InOut::const_iterator it = shaderInputs.begin(), itEnd = shaderInputs.end(); it != itEnd; ++it)
					{
						...
						if (varyingIt != varyingMap.end())
						{
							...
							{
								preprocessor.writef(
									"attribute %s %s %s %s;\n", var.m_precision.c_str(), var.m_interpolation.c_str(), var.m_type.c_str(), name);
							}
							else
							{
								preprocessor.writef(
									"%s varying %s %s %s;\n", var.m_interpolation.c_str(), var.m_precision.c_str(), var.m_type.c_str(), name);
							}
						}
					}
					...
					for (InOut::const_iterator it = shaderOutputs.begin(), itEnd = shaderOutputs.end(); it != itEnd; ++it)
					{
						...
						{
							const Varying &var = varyingIt->second;
							preprocessor.writef("%s varying %s %s;\n", var.m_interpolation.c_str(), var.m_type.c_str(), var.m_name.c_str());
						}
					}
				}
				else
				{
					...
					if (profile->lang == ShadingLang::HLSL && profile->id < 400)
					{
						preprocessor.writef(
							"#define centroid\n"
							"#define flat\n"
					...
					*const_cast<char *>(entry.getPtr() + 4) = '_';
					...
					if ('f' == _options.shaderType)
					{
						...
						if (0 == numFragData)
						{
							...
							if (!insert.isEmpty() && profile->id < 400 && !hasFragColor)
							{
						...
						for (InOut::const_iterator it = shaderInputs.begin(), itEnd = shaderInputs.end(); it != itEnd; ++it)
						{
							...
							{
								const Varying &var = varyingIt->second;
								preprocessor.writef(" \\\n\t%s%s %s %s : %s", arg++ > 0 ? ", " : "  ", interpolationDx11(var.m_interpolation.c_str()), var.m_type.c_str(), var.m_name.c_str(), var.m_semantics.c_str());
							}
						}
					...
					else if ('v' == _options.shaderType)
					{
						...
						for (InOut::const_iterator it = shaderOutputs.begin(), itEnd = shaderOutputs.end(); it != itEnd; ++it)
						{
							...
							if (varyingIt != varyingMap.end())
							{
								const Varying &var = varyingIt->second;
								preprocessor.writef(
									"\t%s %s %s : %s;\n", interpolationDx11(var.m_interpolation.c_str()), var.m_type.c_str(), var.m_name.c_str(), var.m_semantics.c_str());
								preprocessor.writef(
				...
				if (preprocessor.run(input))
				{
...

					{
						...
						if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
						{
							...
							const bool usesBitsToEncoders = true && _options.shaderType == 'f' && !bx::findIdentifierMatch(preprocessedInput, s_bitsToEncoders).isEmpty();

							if (!bx::strFind(preprocessedInput, "layout(std430").isEmpty() || !bx::strFind(preprocessedInput, "image2D").isEmpty() || usesBitsToEncoders)
							{
								if (profile->lang == ShadingLang::GLSL && glsl_profile < 430)
								{
									glsl_profile = 430;
								}
							...
							if (glsl_profile < 400)
							{
								const bool usesTextureLod = false || !bx::findIdentifierMatch(input, s_ARB_shader_texture_lod).isEmpty() || !bx::findIdentifierMatch(input, s_EXT_shader_texture_lod).isEmpty();

								const bool usesGpuShader5 = true && _options.shaderType != 'f' && !bx::findIdentifierMatch(input, s_ARB_gpu_shader5).isEmpty();
								...
								if (profile->lang != ShadingLang::ESSL)
								{
									const bool need130 = (120 == glsl_profile && (false || !bx::findIdentifierMatch(input, s_130).isEmpty() || usesInterpolationQualifiers || usesTexelFetch || usesUnsignedVecs));
							...
							if ((profile->lang == ShadingLang::GLSL && glsl_profile > 400) || (profile->lang == ShadingLang::ESSL && glsl_profile > 300))
							{
								code += preprocessor.m_preprocessed;
...
extern "C"
{
	int cpp_main(int _argc, const char *_argv[])
	{
